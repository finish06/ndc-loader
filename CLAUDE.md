...
go vet ./...                         # Vet check
make docs                            # Regenerate Swagger spec (docs/swagger/)
make k6-smoke | k6-load | k6-spike | k6-soak   # Performance tests against staging
```
...
ndc-loader/
  ...
  tests/
    ...
    k6/                 # k6 performance/load tests (staging)
    screenshots/        # Visual verification artifacts
  specs/                # Feature specifications
  docs/
    prd.md              # Product Requirements Document
    architecture-diagrams.md  # Mermaid flow + ER diagrams (generated by /add:docs)
    api/endpoints.md    # REST endpoint reference
    data-model.md       # Schema and join semantics
    deployment.md       # Local/staging/prod deploy guide
    swagger/            # Generated OpenAPI spec (swag init)
    plans/              # Implementation plans
    milestones/         # Milestone tracking
  .add/                 # ADD methodology config, learnings, docs manifest
  docker-compose.yml    # Local dev environment
  Dockerfile            # Production container
```

### Environments (tier 3 — full pipeline)

- **Local:** `docker-compose up` — PostgreSQL + ndc-loader on http://localhost:8081
- **Dev:** Push to `dev` branch — unit + integration tests
- **Staging:** Push to `staging` branch — e2e + performance (k6) tests; image `:beta` deployed via webhook
- **Production:** Merge to `main` — self-hosted homelab (dockerhub.calebdunn.tech); requires approval
